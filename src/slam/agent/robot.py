...
import src.slam.agent.agent as agent
import src.slam.agent.sensor as sensor
import src.slam.common.datapoint as datapoint
import src.slam.common.geometry as geometry
import src.slam.planner.action as action
import src.slam.planner.planner as planner
import src.slam.ssocket as ssocket
...
class LegoRobot(Robot):
    ...
    def init_sensor(self):
        self.scanner = sensor.LegoUSSensor(self.observation_queue, self.socket,
                                           self.view_angle,
                                           self.scanning_precision,
                                           self.robot_size / 2)
