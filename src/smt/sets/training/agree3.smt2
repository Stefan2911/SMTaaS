...
(declare-fun phase$1 () phase_state$type)
(declare-fun crashing$1 () Nodes$t$type)
(declare-fun crashed () Nodes$t$type)
(declare-fun comp_done () (Array node$type BOOL))
(declare-fun messages$1 () (Array node$type (Array node$type Values$t$type)))
(declare-fun recv_done$1 () (Array node$type (Array node$type BOOL)))
(declare-fun phase () phase_state$type)
(declare-fun round$1 () Int)
(declare-fun clean () clean_state$type)
(declare-fun nondet$1 () (Array node$type (Array node$type BOOL)))
(declare-fun nondet$2 () Nodes$t$type)
(declare-fun clean$1 () clean_state$type)
(declare-fun crashing () Nodes$t$type)
(declare-fun round () Int)
(declare-fun send_done () (Array node$type (Array node$type BOOL)))
(declare-fun crashed$1 () Nodes$t$type)
(declare-fun send_done$1 () (Array node$type (Array node$type BOOL)))
(declare-fun comp_done$1 () (Array node$type BOOL))
(declare-fun deliver_message$1 () (Array node$type (Array node$type BOOL)))
(declare-fun mk_array_8 () (Array node$type (Array node$type Values$t$type)))
(assert (forall ((mk_array_8_index node$type)) (= (select mk_array_8 mk_array_8_index) null_message_set) ))
(assert (not (=> (=> (= phase init_phase) (forall ((n node$type) (m node$type)) (not (= (select (select send_done n) m) Truth)) )) (=> (or (and (= phase init_phase) (forall ((n node$type)) (= (select init_done n) Truth) )) (and (= phase comp_phase) (forall ((n node$type)) (= (select comp_done n) Truth) ))) (=> (= phase$1 send_phase) (=> (= send_done$1 empty_node_pair_set) (=> (= recv_done$1 empty_node_pair_set) (=> (= comp_done$1 empty_node_set) (=> (= crashed$1 (Nodes$union crashed crashing)) (=> (= crashing$1 nondet$2) (=> (= (Nodes$disjoint crashed$1 crashing$1) Truth) (=> (<= (+ (Nodes$cardinality crashed$1) (Nodes$cardinality crashing$1)) t) (=> (= deliver_message$1 nondet$1) (=> (and (forall ((p node$type)) (=> (= (Nodes$mem p crashed$1) Truth) (= (is_silent p deliver_message$1) Truth)) ) (forall ((p node$type)) (=> (= (is_faulty p deliver_message$1) Truth) (= (Nodes$mem p (Nodes$union crashed$1 crashing$1)) Truth)) )) (=> (= messages$1 mk_array_8) (=> (= clean$1 (ite (= clean before) (ite (not (= crashing$1 Nodes$empty)) before active) after)) (=> (= round$1 (+ round 1)) (=> (= phase$1 init_phase) (forall ((n node$type) (m node$type)) (not (= (select (select send_done$1 n) m) Truth)) )))))))))))))))))))
(check-sat)
(exit)
